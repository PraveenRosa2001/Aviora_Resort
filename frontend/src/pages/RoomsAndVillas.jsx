...
function VillaSkeleton() {
  return (
    <div className="bg-surface-container-low/50 rounded-3xl border-2 border-primary/20 p-6 md:p-8 animate-pulse shadow-sm">
...
export default function RoomsAndVillas() {
  ...
  return (
    <div className="pb-28 relative" style={{ backgroundColor: "#F4F1EA" }}>
      <SEO
        ...
        <div className="relative z-10 container-resort text-center text-white max-w-4xl mx-auto px-6">
          <FadeSection>
            <span className="eyebrow-label text-amber-300 bg-white/10 backdrop-blur-md border border-white/20 px-4 py-1.5 rounded-full inline-flex items-center gap-2 mb-4 tracking-widest text-xs font-bold uppercase shadow-sm">
              <Sparkles size={14} className="text-amber-300" /> LUXURY VILLA
              COLLECTION
            </span>
            <h1
            ...
            >
              ...
              rainforest canopy and shimmering lagoon waters. Experience private
              infinity pools, floor-to-ceiling glass vistas, and personalized
              butler service.
            </p>
            ...
            <div className="inline-flex flex-wrap items-center justify-center gap-4 md:gap-8 bg-black/40 backdrop-blur-md border border-white/20 px-6 py-3 rounded-2xl text-xs font-semibold text-white/90 shadow-lg">
              <span className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-amber-400" /> 12
                Private Sanctuaries
              </span>
              <span className="hidden sm:inline text-white/30">•</span>
              <span className="flex items-center gap-2">
                <Waves size={15} className="text-amber-400" /> Private Infinity
                Plunge Pools
              </span>
              <span className="hidden sm:inline text-white/30">•</span>
              <span className="flex items-center gap-2">
                <ShieldCheck size={15} className="text-emerald-400" /> Instant
                Lock Guarantee
              </span>
            </div>
          </FadeSection>
      ...
      <div className="relative w-full overflow-hidden bg-[#F4F1EA] pb-24">
        <PageSketchBackground subtitle="Luxury Sanctuaries &amp; Villa Collection" />
        <div className="relative z-10 pt-16">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                {SANCTUARY_PILLARS.map((pillar, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className="p-6 bg-surface-container-low/50 border-2 border-primary/20 rounded-2xl shadow-xs hover:border-primary/50 transition-all duration-300 hover:-translate-y-1 flex flex-col justify-between group"
                    >
                      <div>
          ...
          <section className="container-resort max-w-6xl mx-auto mb-16">
            <FadeSection delay={0.15}>
              <div className="p-6 sm:p-8 bg-surface-container-low/50 border-2 border-primary/30 rounded-3xl shadow-xl space-y-6">
                ...
                {/* Top Row: Date & Guest Parameters (Expanded to 3 balanced columns) */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6 items-end">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 bg-white/10 border border-outline-variant/60 rounded-xl text-xs sm:text-sm font-semibold text-black/80 focus:outline-none focus:border-white/60 focus:ring-2 focus:ring-white/20 shadow-xs transition-all [color-scheme:light] [&::-webkit-calendar-picker-indicator]:opacity-100 cursor-pointer"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:gap-3">
                      {/* Adults Stepper */}
                      <div className="flex items-center justify-between px-3.5 py-2.5 bg-surface-container-low/50 border border-outline-variant/40 rounded-xl text-xs font-bold shadow-xs">
                        <button
                          type="button"
                          onClick={() =>
                            dispatch(setAdults(Math.max(1, adults - 1)))
                          }
                          className="w-7 h-7 rounded-lg bg-white/90 border border-outline-variant/60 flex items-center justify-center text-xs font-bold text-deep-wood hover:bg-primary hover:text-white transition-colors cursor-pointer shrink-0"
                          aria-label="Decrease adults"
                        >
                          -
                        </button>
                        <span className="text-deep-wood font-bold px-1 truncate">
                ...
                {/* Bottom Row: Category Pills, View Select, Price Slider & Sort (Single Row - No Scrollbar) */}
                <div className="pt-4 border-t border-outline-variant/20 flex items-center justify-between gap-2 lg:gap-4">
                  {/* Category Pills */}
                  <div className="flex items-center gap-1 p-1 bg-surface-container-high rounded-xl border border-primary/20 shrink-0">
                    {[
                      {
                        id: "all",
                        label: "All Sanctuaries",
                        shortLabel: "All",
                      },
                      {
                        id: "canopy",
                      ...
                      <button
                        ...
                        className={[
                          "px-2 sm:px-2.5 xl:px-3 py-1.5 text-[10px] sm:text-[11px] xl:text-xs font-bold uppercase tracking-wider rounded-lg transition-all cursor-pointer whitespace-nowrap",
                  ...
                  <div className="flex items-center gap-2.5 sm:gap-3 xl:gap-4 text-xs font-semibold text-deep-wood shrink-0">
                    {/* View filter */}
                    <div className="flex items-center gap-1 sm:gap-1.5">
                      <span className="text-deep-wood/70 uppercase tracking-wider text-[10px] sm:text-[11px] font-bold whitespace-nowrap">
                        View:
                      </span>
                      <select
                        value={viewFilter}
                        onChange={(e) => setViewFilter(e.target.value)}
                        className="px-2 sm:px-2.5 py-1.5 bg-white border border-outline-variant/40 rounded-lg text-xs text-deep-wood font-semibold focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary shadow-xs cursor-pointer capitalize"
                      >
                    ...
                    <div className="flex items-center gap-1.5 sm:gap-2">
                      ...
                      <input
                        ...
                        className="w-16 sm:w-20 xl:w-24 accent-primary cursor-pointer h-1.5 bg-surface-container-high rounded-lg"
                    ...
                    <div className="flex items-center gap-1 sm:gap-1.5">
                      ...
                      <select
                        ...
                        className="px-2 sm:px-2.5 py-1.5 bg-white border border-outline-variant/40 rounded-lg text-xs text-deep-wood font-semibold focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary shadow-xs cursor-pointer"
                      >
          ...
          <section className="container-resort max-w-6xl mx-auto mb-20">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-8 px-2">
              ...
              <span className="text-xs text-emerald-800 bg-emerald-50 border border-emerald-200/80 px-3.5 py-1.5 rounded-full font-bold inline-flex items-center gap-1.5 shadow-xs">
                <ShieldCheck size={15} className="text-emerald-700" /> Free
                Cancellation up to 48hrs prior
              </span>
            </div>
            ...
            {/* Empty Results State */}
            {showEmpty && (
              <div className="py-20 text-center border-2 border-primary/20 rounded-3xl bg-surface-container-low/50 p-8 shadow-md">
                ...
                {displayVillas.map((villa, idx) => {
                  ...
                  return (
                    ...
                    >
                      <article
                        className={[
                          "bg-surface-container-low/50 border-2 rounded-3xl overflow-hidden shadow-md transition-all duration-300 grid grid-cols-1 lg:grid-cols-12 gap-0 group",
                          soldOut
                      ...
                      >
                        ...
                        <div className="p-6 md:p-8 lg:col-span-5 flex flex-col justify-between border-b lg:border-b-0 lg:border-r border-outline-variant/20">
                          <div>
                            ...
                            <div className="flex items-center gap-2 mb-2 flex-wrap">
                              ...
                              >
                                ({villa.reviewCount || 48} verified guest
                                reviews)
                              </button>
                            </div>
                            ...
                            {/* Key Specs Pill Grid */}
                            <div className="grid grid-cols-2 gap-2 p-3 bg-surface-container-low/50 border border-primary/20 rounded-2xl text-xs font-semibold text-deep-wood mb-4">
                              <div className="flex items-center gap-2">
                                ...
                                />
                                <span>
                                  {villa.size} {villa.sizeUnit || "sqm"} Area
                                </span>
                              </div>
                              <div className="flex items-center gap-2">
                                <Users
                          ...
                          <div className="pt-3 border-t border-outline-variant/20 space-y-1.5">
                            <div className="flex items-center gap-2 text-xs font-bold text-emerald-800">
                              ...
                              />
                              <span>
                                Complimentary Gourmet Breakfast &amp; Butler
                              </span>
                            </div>
                        ...
                        <div className="p-6 md:p-8 lg:col-span-3 bg-surface-container-low/50/40 flex flex-col justify-between items-end text-right">
                          <div className="w-full">
                            ...
                            <span className="text-[11px] text-deep-wood/55 block font-medium">
                              Plus {TAX_PERCENT_LABEL} government taxes &amp;
                              service charge
                            </span>
                            ...
                            {/* Guarantee Box */}
                            <div className="mt-4 p-3.5 bg-surface-container-low/50 border border-primary/20 rounded-2xl text-left space-y-1 shadow-xs">
                              <span className="text-xs font-bold text-secondary flex items-center gap-1.5">
                                <Shield size={13} /> Flexible Guarantee:
                              </span>
                              <p className="text-[11px] text-deep-wood/70 leading-relaxed font-medium">
                                100% free cancellation up to 48 hours prior to
                                check-in date.
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="w-full mt-6 space-y-2.5">
                            ...
                            >
                              <span>
                                {soldOut ? "Sold Out" : "Reserve Villa"}
                              </span>
                              <Sparkles size={14} />
                            </button>
