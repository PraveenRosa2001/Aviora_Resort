...
//           <div className="p-4 sm:p-6 border-b border-outline-variant/20 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-surface-container-low/50/40">
//             {/* View Tabs */}
...
//                 <thead>
//                   <tr className="border-b border-outline-variant/20 bg-surface-container-low/50 text-deep-wood/70 font-bold uppercase tracking-wider text-[12px]">
...
//                       <tr key={res.referenceId} className="hover:bg-surface-container-low/50/50 transition-colors">
//                         {/* Ref Code */}
...
import AdminInventoryTab from "../pages/AdminInventoryTab";
import AdminDiningTab from "../pages/AdminDiningTab";
import { StayDocuments } from "../components/documents/ReservationDocuments";
import ToastAlert from "../components/common/Toast";
...
export default function AdminDashboard() {
  ...
  const handleUpdateStatus = async (refId, newStatus) => {
    try {
      ...
      showToast(
        res?.message || `Reservation ${refId} moved to ${newStatus}.`,
        "success",
      );
    } catch (err) {
      showToast(
    ...
    <div className="pt-28 pb-24 min-h-screen bg-surface text-on-surface">
      ...
      <div className="container-resort max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-white rounded-2xl p-5 border border-outline-variant/30 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-bold uppercase tracking-wider text-deep-wood/60">
                Estate Occupancy
              </span>
              <div className="w-8 h-8 rounded-lg bg-secondary-container/30 text-secondary flex items-center justify-center">
                <TrendingUp size={16} />
          ...
          <div className="bg-white rounded-2xl p-5 border border-outline-variant/30 shadow-sm">
            ...
            <p className="text-[12px] text-deep-wood/60 mt-3">
              {kpis?.arrivalsToday ?? 0} arriving today ·{" "}
              {kpis?.departuresToday ?? 0} departing
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl p-5 border border-outline-variant/30 shadow-sm">
            ...
            <p className="text-[12px] text-deep-wood/60 mt-3 flex items-center gap-1">
              <Sparkles size={13} className="text-amber-500" />
              <span>
                Rs.{(kpis?.revenue30Days ?? 0).toLocaleString()} in the last 30
                days
              </span>
            </p>
          </div>
                ...
                <tbody className="divide-y divide-outline-variant/15 text-deep-wood">
                  {filteredReservations.length === 0 ? (
                    <tr>
                      <td
                        colSpan={7}
                        className="py-12 text-center text-deep-wood/50"
                      >
                    ...
                    filteredReservations.map((res) => (
                      <tr
                        key={res.referenceId}
                        className="hover:bg-surface-container-low/50/50 transition-colors"
                      >
                        {/* Ref Code */}
                        <td className="py-4 px-5">
                          <span className="font-mono font-bold text-primary bg-primary/10 px-2 py-1 rounded-md text-[12px]">
                        ...
                        {/* Villa */}
                        <td className="py-4 px-5">
                          <div className="font-semibold text-deep-wood">
                            {res.villaName}
                          </div>
                          <div className="text-[10px] text-deep-wood/60">
                            {res.adults} Adults{" "}
                        ...
                        <td className="py-4 px-5 text-right">
                          <div className="inline-flex items-center gap-1.5">
                            ...
                            {/* Voucher and the accounting copy. isAdmin is
                                true here, so the Invoice button renders - it
                                prints the tax lines SNAPSHOTTED onto the
                                booking, which is why a reprint after a VAT
                                change still shows what was charged. */}
                            <StayDocuments booking={res} isAdmin compact />
        ...
        {selectedVoucher && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-deep-wood/80 backdrop-blur-md">
            ...
            >
              ...
              <div className="space-y-3.5 text-xs text-deep-wood">
                <div className="flex justify-between py-1.5 border-b border-outline-variant/10">
                  <span className="font-semibold text-deep-wood/60">
                    Primary Guest:
                  </span>
                  <span className="font-bold">
                    {selectedVoucher.guestInfo?.firstName}{" "}
                    {selectedVoucher.guestInfo?.lastName}
                  </span>
                </div>
                <div className="flex justify-between py-1.5 border-b border-outline-variant/10">
                  <span className="font-semibold text-deep-wood/60">
                  ...
                  <span>
                    {selectedVoucher.guestInfo?.email} •{" "}
                    {selectedVoucher.guestInfo?.phone}
                  </span>
                </div>
                <div className="flex justify-between py-1.5 border-b border-outline-variant/10">
                  <span className="font-semibold text-deep-wood/60">
                ...
                <div className="flex justify-between py-1.5 border-b border-outline-variant/10">
                  ...
                  <span>
                    {selectedVoucher.checkIn} to {selectedVoucher.checkOut} (
                    {selectedVoucher.nights} Nights)
                  </span>
                </div>
                <div className="flex justify-between py-1.5 border-b border-outline-variant/10">
                  <span className="font-semibold text-deep-wood/60">
                  ...
                  <span>
                    {selectedVoucher.adults} Adults,{" "}
                    {selectedVoucher.children || 0} Children
                  </span>
                </div>
                <div className="flex justify-between py-1.5 border-b border-outline-variant/10">
                  <span className="font-semibold text-deep-wood/60">
